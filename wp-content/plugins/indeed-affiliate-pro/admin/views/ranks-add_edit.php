<?php
global $wpdb;
 $row_1 = $wpdb->get_results(  "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
WHERE table_name = 'wp_customer_say' AND column_name = 'header_static_text_left'"  );

if(empty($row_1)){
   $wpdb->query("ALTER TABLE wp_uap_ranks ADD header_static_text_left TEXT NOT NULL AFTER label");
}



 $row_11 = $wpdb->get_results(  "select COLLATION_NAME from information_schema.columns where TABLE_NAME = 'wp_uap_ranks' and COLUMN_NAME = 'header_static_text_left'"  );

 //print_r($row_11);
 
 if($row_11->COLLATION_NAME!='utf8_unicode_ci')
 {
   $wpdb->query("ALTER TABLE wp_uap_ranks CHANGE header_static_text_left header_static_text_left TEXT CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL");
...
$row_2 = $wpdb->get_results(  "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
WHERE table_name = 'wp_customer_say' AND column_name = 'header_static_text_right'"  );

if(empty($row_2)){
   $wpdb->query("ALTER TABLE wp_uap_ranks ADD header_static_text_right TEXT NOT NULL AFTER header_static_text_left");
}


$row_22 = $wpdb->get_results(  "select COLLATION_NAME from information_schema.columns where TABLE_NAME = 'wp_uap_ranks' and COLUMN_NAME = 'header_static_text_right'"  );

 //print_r($row_11);
 
 if($row_22->COLLATION_NAME!='utf8_unicode_ci')
 {
   $wpdb->query("ALTER TABLE wp_uap_ranks CHANGE header_static_text_right header_static_text_right TEXT CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL");
...
<form action="<?php echo $data['url-manage'];?>" method="post">
				...
				<div class="uap-inside-item">	
					...
					<div class="row">
						<div class="col-xs-4">			
							<div class="form-group">
								<label class="control-label"><?php _e('Header Left Static Text', 'uap')?></label>
								<textarea name="header_static_text_left" class="form-control text-area" cols="30" rows="5" placeholder="<?php _e('Some details...', 'uap');?>"><?php echo $data['metas']['header_static_text_left'];?></textarea>
							</div>
						</div>
					</div>	
				</div>
					...
					<div class="row">
						<div class="col-xs-4">			
							<div class="form-group">
								<label class="control-label"><?php _e('Header Right Static Text', 'uap')?></label>
								<textarea name="header_static_text_right" class="form-control text-area" cols="30" rows="5" placeholder="<?php _e('Some details...', 'uap');?>"><?php echo $data['metas']['header_static_text_right'];?></textarea>
							</div>
						</div>
					</div>	
				</div>
