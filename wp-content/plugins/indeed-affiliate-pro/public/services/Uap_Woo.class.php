...
class Uap_Woo extends Referral_Main{
	...
	public function create_referral($order_id=0){
		...
		if ($this->valid_referral()){
			...
			//$do_math2 = new Affiliate_Referral_Amount(31, $this->source_type, self::$special_payment_type, self::$coupon_code);
			...
			foreach ($items as $item){ /// foreach in lines
				...
				$product_price = get_post_meta($item['product_id'],'_wc_points_earned',true);
				$product_pp = get_post_meta($item['product_id'],'_wc_points_earned',true);
				...
				$temp_amount2 = getReferrerAmount($product_pp,self::$affiliate_id,self::$user_id);// input price, product id
				$temp_amount2=$temp_amount2*$qty;
				//$sum2= $temp_amount-$temp_amount2;
				//$sum += $temp_amount;
				$sum += $temp_amount2;
				//$sum += $sum2;
