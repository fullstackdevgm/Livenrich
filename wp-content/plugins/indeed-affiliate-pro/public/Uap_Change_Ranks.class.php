...
class Uap_Change_Ranks{
	...
	public function __construct($affiliates_ids_in=array()){
		...
		if ($this->ranks_data){
			...
			//krsort($this->ranks_data);// sort array reverse by key
			$affiliates = $indeed_db->get_affiliates_from_referrals($affiliates_ids_in);/// get affiliates with at least one referral verified
			if ($affiliates){
			...
			foreach ($ranks as $key=>$object){ /// LOOP THROUGH RANKS
				if (!empty($object->achieve)){
					...
					if (!empty($achieve) && is_array($achieve)){
						...
						$condition .='){$this->set_new_rank($affiliate_id, $rank_id);} else {$this->set_break();}';
						eval($condition);
						if ($this->do_break){
							break;
						}
	...
	private function set_new_rank($affiliate_id, $rank_id){
		...
		/// PAY THE BONUS
		//$rank_data = $indeed_db->get_rank($rank_id);
		/*
		if ($this->bonus_enabled && !empty($rank_data['bonus'])){
			/// we must pay...
			if (!class_exists('Referral_Main')){
				require_once UAP_PATH . 'public/Referral_Main.class.php';
			}
			$object = new Referral_Main(0, $affiliate_id);
			$object->pay_bonus($rank_data['bonus'], $rank_data['label']);
			$uid = $indeed_db->get_uid_by_affiliate_id($affiliate_id);
			uap_send_user_notifications($uid, 'rank_change', $rank_id);//send notification to user
			uap_send_user_notifications($uid, 'admin_on_aff_change_rank', $rank_id);//send notification to admin
		}
