...
if (!class_exists('Affiliate_Account_Page')){
	class Affiliate_Account_Page{
		...
		private function head(){
			...
			require_once UAP_PATH . 'public/views/account_page-header.php';
		}
		...
		private function print_top_messages(){
			...
			require_once UAP_PATH . 'public/views/account_page-top_messages.php';
		}
		...
		private function footer(){
			...
			require_once UAP_PATH . 'public/views/account_page-footer.php';
		}
		...
		private function run_affiliate_link(){
			...
			.banner-content{display:none;}
				...
				<div class="banner-image section-left" style="display:inline-block; float:left; width:50%;">
				<?php 					
					//$this->run_banners();
				?>		
				</div>
    ...
    jQuery(function() {
        jQuery(".uap-banner").click(function() {
            jQuery(".uap-banner").removeClass('active-uap-banner');
            jQuery(this).addClass('active-uap-banner');
			
			var imagelink = jQuery(this).find('img').attr('src');
			jQuery('#input_3_8').val(imagelink);
			
			var reflink = jQuery(this).find('.uap-ref-link').html();
			jQuery('#input_3_10').val(reflink);
			
			
			
        });
		
		var uname='<?php echo $user_info->user_login;?>';
		 jQuery("#input_3_11").keyup(function() {
			 var slink=jQuery('#input_3_11').val();
			 if(slink.indexOf("?")<0)
				  convert_link=slink+'?ref='+uname;
			 else
				 convert_link=slink+'&ref='+uname;
			
			 jQuery('#input_3_13').val(convert_link);
				...
				<script>
					...
					jQuery( document ).ready(function() {
						var reflink = jQuery(".uap-account-url a").attr('href');
						jQuery('#input_3_10').val(reflink);
						
						/*jQuery('.uap-banner-img img').click(function(){
				...
				<div class="section-right" style="float:none; display:block; width:100%;">
				<?php
					echo do_shortcode('[gravityform id=13 title=false description=false ajax=true tabindex=49]');
				?>	
				</div>
				<div class="section-bottom" style="width:100%;">
					<?php
						$this->print_top_messages();
						require_once UAP_PATH . 'public/views/account_page-generate_links.php';
					?>
				</div>
				<div class="referal-history-section">
				<?php
					$this->run_referrals_history_link_page();
				?>
				</div>
			</div>			
				<?php
		...
		private function run_overview(){
			...
			$this->print_top_messages();
			require_once UAP_PATH . 'public/views/account_page-overview.php';
		}
		...
		private function run_banners(){
			...
			$this->print_top_messages();
			require_once UAP_PATH . 'public/views/account_page-banners.php';
		}
		...
		private function run_change_password(){
				...
				if (empty($_POST['old_pass']) || empty($_POST['pass1']) || empty($_POST['pass2'])){
					...
					if ( $user && wp_check_password( $_POST['old_pass'], $user->data->user_pass,  $this->uid) ){
						if ($_POST['pass1']==$_POST['pass2']){
							...
							$data['success'] = __("Your have change your Password!", 'uap');
						} else {
			...
			$this->print_top_messages();
			require_once UAP_PATH . 'public/views/account_page-change_password.php';
		}
		...
		private function run_edit_account(){
			...
			$this->print_top_messages();
			require_once UAP_PATH . 'public/views/account_page-edit_account.php';
		}
		...
		private function run_campaigns(){
			...
			$this->print_top_messages();
			require_once UAP_PATH . 'public/views/account_page-campaigns.php';
		}
		...
		private function run_visits(){
			...
			$this->print_top_messages();
			require_once UAP_PATH . 'public/views/account_page-list_visits.php';
		}
		...
		private function run_campaign_reports(){
			...
			$this->print_top_messages();
			require_once UAP_PATH . 'public/views/account_page-campaign_reports.php';
		}
		...
		private function run_referrals(){
			...
			$this->print_top_messages();
			require_once UAP_PATH . 'public/views/account_page-referrals.php';
		}
		...
		private function run_payments(){
			...
			$this->print_top_messages();
			require_once UAP_PATH . 'public/views/account_page-payments.php';
		}
		...
		private function run_reports(){
			...
			$this->print_top_messages();
			require_once UAP_PATH . 'public/views/account_page-reports.php';
		}
		...
		private function run_payment_settings(){
			...
			$this->print_top_messages();
			require_once UAP_PATH . 'public/views/account_page-payment_settings.php';
		}
		...
		if (isset($_REQUEST['u_sts']) && $_REQUEST['u_sts']!=-1){
			...
			//$data['items'] = $indeed_db->get_referrals($limit, $offset, FALSE, '', '', $where);
                       ...
                       // print_r( $indeed_db->get_referrals_with_pp($limit, $offset, FALSE, '', '', $where));
                        
                        
                        /* refral purchase history */
                        
                        $data['items']= $indeed_db->get_referrals_with_pp($limit, $offset, FALSE, 'date', 'DESC', $where);
			...
			$this->print_top_messages();
			require_once UAP_PATH . 'public/views/account_page-referrals_history.php';
		
                        //require_once UAP_PATH . 'public/views/account_page-history_affiliate_link.php';
	
                        }

		
		private function run_referrals_history_link_page(){
			/*
			 * @param none
			 * @return string
			 */
			...
			$limit = 10;
			//$url = add_query_arg('uap_aff_subtab', 'referrals_history', $this->account_page_base_url);//// referrals
			$url = add_query_arg('uap_aff_subtab', 'affiliate_link', $this->account_page_base_url);//// referrals
			$where = array();
			if (!empty($_REQUEST['udf']) && !empty($_REQUEST['udu'])){
				$where[] = " r.date>'" . $_REQUEST['udf'] . "' ";
				$where[] = " r.date<'" . $_REQUEST['udu'] . "' ";
			...
			$data['items'] = $indeed_db->get_referrals($limit, $offset, FALSE, '', '', $where);
			$data['ppitems'] = $indeed_db->get_referrals_with_pp($limit, $offset, FALSE, '', '', $where);
			...
			$this->print_top_messages();
			require_once UAP_PATH . 'public/views/account_page-help.php';
		}
		...
		private function print_coupons(){
			...
			$data['currency'] = get_option('uap_currency');
			require_once UAP_PATH . 'public/views/account_page-coupons.php';
		}
		...
		private function print_custom_affiliate_slug(){
			...
			require_once UAP_PATH . 'public/views/account_page-custom_affiliate_slug.php';
		}
		...
		private function print_mlm_page(){
			...
			require_once UAP_PATH . 'public/views/account_page-mlm.php';
		}
		...
		private function print_wallet(){
			...
			if (empty($_GET['add_new'])){
				...
				require_once UAP_PATH . 'public/views/wallet-add_new.php';
			}
		...
		private function print_referral_notifications(){
			...
			$this->print_top_messages();
			require_once UAP_PATH . 'public/views/account_page-referral_notifications.php';
		}
		...
		private function print_source_details(){
			...
			require_once UAP_PATH . 'public/views/account_page-source_details.php';
		}
		...
		private function print_pushover_notifications(){
			...
			$this->print_top_messages();
			require UAP_PATH . 'public/views/account_page-pushover_notifications.php';
		}
		...
		private function run_simple_links(){
			...
			$this->print_top_messages();
			require_once UAP_PATH . 'public/views/account_page-simple_links.php';
		}
		...
		private function run_custom_tab($tab=''){
			...
			require UAP_PATH . 'public/views/account_page-custom_tab.php';
		}
