...
if (!class_exists('Uap_Db')){
	class Uap_Db{
					...
					foreach ($data as $obj){
						if ($obj->id){
							$return_arr[$obj->id] = array(
														...
														'rank_color' => isset($rank_data['color']) ? $rank_data['color'] : '',
							...
							if ($return_arr[$obj->id]['rank_id']){
								$rank_data = $this->get_rank($return_arr[$obj->id]['rank_id']);
								if (!empty($rank_data['label'])){
									$return_arr[$obj->id]['rank_label'] = $rank_data['label'];
								}
							}
						}
					}
		...
		public function get_affiliates_from_referrals($affiliates_ids_in=array()){
			/*
			 * @param array
			 * @return array
			 */
...
<div><strong>Refuse Referrals :</strong> {refuse_referrals}</div>
			...
			if ($post_data){
				...
				if (!empty($post_data['id'])){
					...
					if ($exists){
						...
						$wpdb->query("UPDATE $table SET
											...
											label='" . $post_data['label'] . "',
                                                                                        header_static_text_left='" . $post_data['header_static_text_left'] . "',
                                                                                        header_static_text_right='" . $post_data['header_static_text_right'] . "',
					...
					$wpdb->query("INSERT INTO $table VALUES(NULL,
									'" . $post_data['slug'] . "',
									'" . $post_data['label'] . "',
                                                                        '" . $post_data['header_static_text_left'] . "',
                                                                        '" . $post_data['header_static_text_right'] . "',
		...
		public function get_referrals($limit=-1, $offset=-1, $count=FALSE, $order_by='date', $order_type='DESC', $where_conditions=array()){
			/*
			...
			return $return;
                ...
                public function get_referrals_with_pp($limit=-1, $offset=-1, $count=FALSE, $order_by='date', $order_type='DESC', $where_conditions=array()){
			/*
			 * @param int, int, boolean
			 * @return array/int
			 */
			...
			$table_affiliates = $wpdb->prefix . 'uap_affiliates';
			$table_users = $wpdb->base_prefix . 'users';
                        $table_reward_point=$wpdb->prefix . "wc_points_rewards_user_points";
			if ($count){
				$q = "SELECT COUNT(*) as c FROM $table r";
				$q .= " INNER JOIN $table_affiliates a ON a.id=r.affiliate_id";
				$q .= " INNER JOIN $table_users u ON u.ID=a.uid";
                                 $q .= " INNER JOIN $table_reward_point up ON r.reference.=up.order_id";
				$q .= " WHERE 1=1";
				if (!empty($where_conditions)){
					foreach ($where_conditions as $condition){
						$q .= " AND " . $condition ;
				...
				$q = "SELECT r.*,up.order_id,up.points FROM $table r";
				$q .= " INNER JOIN $table_affiliates a ON a.id=r.affiliate_id";
				$q .= " INNER JOIN $table_users u ON u.ID=a.uid";
                                $q .= " INNER JOIN $table_reward_point up ON r.reference=up.order_id";
				$q .= " WHERE 1=1";
			...
			if (!empty($post_data)){
				...
				if (!empty($post_data['id'])){
					...
					if (isset($wpdb->insert_id)){
						$user_id=$this->get_uid_by_affiliate_id($post_data['affiliate_id']);
					  updateWalletBalanceReferral($user_id,$post_data['amount']);
					}
		...
		public function get_stats_for_payments($affiliate_id=0){
			...
			$temp_data = $wpdb->get_row($q);*/
			$user_id=$this->get_uid_by_affiliate_id($affiliate_id);
			
			$array['unpaid_payments_value'] = getCurrentUserRewardPointWallet($user_id);
		...
		function get_post_id_by_cpt_name($custom_post_type='', $post_name=''){
			/*
			 * @param string, string
