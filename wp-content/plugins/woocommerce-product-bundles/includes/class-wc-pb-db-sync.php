...
class WC_PB_DB_Sync {
	...
	public static function sync() {
		...
		if ( self::$sync_needed ) {
			...
			if ( ! empty( $ids ) ) {
				...
				if ( ! self::$sync_task_runner->is_running() ) {
					...
					if ( false === get_site_transient( 'wp_wc_pb_db_sync_task_runner_manual_lock' ) ) {
						set_site_transient( 'wp_wc_pb_db_sync_task_runner_manual_lock', microtime(), 2 );
					}
		...
		// Give background processing a chance to work before considering a manual run...
		} elseif ( false === get_site_transient( 'wp_wc_pb_db_sync_task_runner_manual_lock' ) ) {
			...
			if ( self::$sync_task_runner->is_queued() && ! self::$sync_task_runner->is_running() ) {
				...
				do_action( 'wp_wc_pb_db_sync_task_runner_cron' );
			}
		}
	}
