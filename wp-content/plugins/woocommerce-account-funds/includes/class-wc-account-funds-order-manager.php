...
class WC_Account_Funds_Order_Manager {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_payment_complete', array( $this, 'maybe_increase_funds' ) ); 
		...
		if ( $customer_id && ! get_post_meta( $order_id, '_funds_deposited', true ) ) {
			foreach ( $items as $item ) {
				...
				if ( $product->is_type( 'deposit' ) || $product->is_type( 'topup' ) ) {
					...
					//sudip custom credit added here
					//$funds = get_field( "epoint_value", $item['product_id']);
					$funds = get_post_meta($item['product_id'], "wccaf_epoint_value", true )*$item['quantity'];
