...
class WC_Points_Rewards_Order {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_payment_complete', array( $this, 'add_points_earned' ), 10 );
		...
		add_action( 'woocommerce_order_status_pending_to_completed', array( $this, 'add_points_earned' ) );
		...
		add_action( 'woocommerce_payment_complete', array( $this, 'maybe_deduct_redeemed_points' ), 9 );
	...
	public function add_points_earned( $order ) {
      ...
      if ($payment_method=='accountfunds') {
			return;
		}
		if ( '' !== $points ) {
			return;
