...
?>
			<li class="recurring-totals">
				<span class="left"><?php esc_html_e( 'Recurring Totals', 'woocommerce-subscriptions' );?></span>
			</li>
			...
			<?php foreach ( $recurring_carts as $recurring_cart_key => $recurring_cart ) : ?>
				<?php if ( 0 == $recurring_cart->next_payment_date ) : ?>
					<?php continue; ?>
				<?php endif; ?>
				<li class="cart-subtotal recurring-total">
					<?php if ( $display_th ) : $display_th = false; ?>
						<span class="left" rowspan="<?php echo esc_attr( $carts_with_multiple_payments ); ?>"><?php esc_html_e( 'Subtotal', 'woocommerce-subscriptions' ); ?></span>
						<span class="right" data-title="<?php esc_attr_e( 'Subtotal', 'woocommerce-subscriptions' ); ?>"><?php wcs_cart_totals_subtotal_html( $recurring_cart ); ?></span>
					<?php else : ?>
						<span class="right"><?php wcs_cart_totals_subtotal_html( $recurring_cart ); ?></span>
					<?php endif; ?>
				</li>
			<?php endforeach; ?>
			<?php $display_th = true; ?>
				...
				<?php foreach ( $recurring_carts as $recurring_cart_key => $recurring_cart ) : ?>
					...
					<?php foreach ( $recurring_cart->get_coupons() as $recurring_code => $recurring_coupon ) : ?>
						<?php if ( $recurring_code !== $code ) { continue; } ?>
							<li class="cart-discount coupon-<?php echo esc_attr( $code ); ?> recurring-total">
								<?php if ( $display_th ) : $display_th = false; ?>
									<span class="left" rowspan="<?php echo esc_attr( $carts_with_multiple_payments ); ?>"><?php wc_cart_totals_coupon_label( $coupon ); ?></span>
									<span class="right" data-title="<?php wc_cart_totals_coupon_label( $coupon ); ?>"><?php wcs_cart_totals_coupon_html( $recurring_coupon, $recurring_cart ); ?></span>
								<?php else : ?>
									<span class="right"><?php wcs_cart_totals_coupon_html( $recurring_coupon, $recurring_cart ); ?></span>
								<?php endif; ?>
							</li>
					<?php endforeach; ?>
				<?php endforeach; ?>
		...
		<?php foreach ( $recurring_carts as $recurring_cart_key => $recurring_cart ) : ?>
			...
			<?php foreach ( $recurring_cart->get_fees() as $recurring_fee ) : ?>
				<li class="fee recurring-total">
					<span class="left"><?php echo esc_html( $recurring_fee->name ); ?></span>
					<span class="right"><?php wc_cart_totals_fee_html( $recurring_fee ); ?></span>
				</li>
			<?php endforeach; ?>
		<?php endforeach; ?>
				...
				<?php foreach ( WC()->cart->get_taxes() as $tax_id => $tax_total ) : ?>
					<?php foreach ( $recurring_carts as $recurring_cart_key => $recurring_cart ) : ?>
						...
						<?php foreach ( $recurring_cart->get_tax_totals() as $recurring_code => $recurring_tax ) : ?>
							...
							<li class="tax-rate tax-rate-<?php echo esc_attr( sanitize_title( $recurring_code ) ); ?> recurring-total">
								<?php if ( $display_th ) : $display_th = false; ?>
									<span class="left"><?php echo esc_html( $recurring_tax->label ); ?></span>
									<span class="right" data-title="<?php echo esc_attr( $recurring_tax->label ); ?>"><?php echo wp_kses_post( wcs_cart_price_string( $recurring_tax->formatted_amount, $recurring_cart ) ); ?></span>
								<?php else : ?>
									<span class="left"></span>
									<span class="right"><?php echo wp_kses_post( wcs_cart_price_string( $recurring_tax->formatted_amount, $recurring_cart ) ); ?></span>
								<?php endif; ?>
							</li>
						<?php endforeach; ?>
					<?php endforeach; ?>
			...
			<?php else : ?>
				...
				<?php foreach ( $recurring_carts as $recurring_cart_key => $recurring_cart ) : ?>
					<?php if ( 0 == $recurring_cart->next_payment_date ) : ?>
						<?php continue; ?>
					<?php endif; ?>
					<li class="tax-total recurring-total">
						<?php if ( $display_th ) : $display_th = false; ?>
							<span class="left"><?php echo esc_html( WC()->countries->tax_or_vat() ); ?></span>
							<span class="right" data-title="<?php echo esc_attr( WC()->countries->tax_or_vat() ); ?>"><?php echo wp_kses_post( wcs_cart_price_string( $recurring_cart->get_taxes_total(), $recurring_cart ) ); ?></span>
						<?php else : ?>
							<span class="left"></span>
							<span class="right"><?php echo wp_kses_post( wcs_cart_price_string( $recurring_cart->get_taxes_total(), $recurring_cart ) ); ?></span>
						<?php endif; ?>
					</li>
				<?php endforeach; ?>
				<?php $display_th = true; ?>
		...
		<?php foreach ( $recurring_carts as $recurring_cart_key => $recurring_cart ) : ?>
			<?php if ( 0 == $recurring_cart->next_payment_date ) : ?>
				<?php continue; ?>
			<?php endif; ?>
			<li class="order-total recurring-total">
				<?php if ( $display_th ) : $display_th = false; ?>
					<span class="left" rowspan="<?php echo esc_attr( $carts_with_multiple_payments ); ?>"><?php esc_html_e( 'Recurring Total', 'woocommerce-subscriptions' ); ?></span>
					<span class="right" data-title="<?php esc_attr_e( 'Recurring Total', 'woocommerce-subscriptions' ); ?>"><?php wcs_cart_totals_order_total_html( $recurring_cart ); ?></span>
				<?php else : ?>
					<span class="right"><?php wcs_cart_totals_order_total_html( $recurring_cart ); ?></span>
				<?php endif; ?>
			</li>
		<?php endforeach; ?>
<?php
