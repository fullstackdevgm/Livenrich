...
function getInviteData()
  {
	  global $wpdb;
	  $table=$wpdb->prefix."uap_invites";
	  $usertable=$wpdb->prefix."users";
	  $user_id=get_current_user_id(); 
	  $sql="Select * from $table where user_id=$user_id and invite_user_id=0 order by created desc";
	  ...
	  $user_ids=array();
		foreach($users as $user)
		{
		  ...
		  $user_ids[$user->user_email]=$user->ID;
		}
		...
		if(count($results)>0)
			foreach($results as $result)
			{
				...
				{
					$data=array('invite_user_id'=>$user_ids[$result->email]);
  ...
  function  saveCustomInvites($email,$page,$url,$mes)
  {
	    global $wpdb;
	  $table=$wpdb->prefix."uap_invites";
  ...
  function resendInviteEmail($post ) {
//	$email => rgar( $entry, '1.3' );
  ...
  function  getRefferalAmount($referral_id)
  {
	  if ($referral_id){
				global $wpdb;
  ...
  function getCurrentUserRewardPoint()
  {
	  
	  global $indeed_db;
$user_id=get_current_user_id(); 
$affiliate_id=getAffiliateID($user_id);


  if ($affiliate_id){
	  $stats = $indeed_db->get_stats_for_payments($affiliate_id);
        return $stats['unpaid_payments_value'];
			/*	global $wpdb;
				$table = $wpdb->prefix . "uap_referrals";
				$q = "SELECT * FROM $table WHERE status='2' AND payment=0 AND affiliate_id=$affiliate_id";
    ...
    function getCurrentUserRewardPointWallet($user_id=null)
     {
	     global $wpdb;
		 if(!$user_id)
		  $user_id=get_current_user_id(); 		
         $resualt = $wpdb->get_row("SELECT balance, currency FROM {$wpdb->base_prefix}woo_wallet_transactions WHERE user_id = {$user_id} ORDER BY transaction_id DESC");
           if ($resualt) {
                    return $resualt->balance;
                }
				
				return 0;
    }
  
  function updateWalletBalanceReferral($user_id,$amount,$type='credit',$details=null)
  {
            ...
            $balance = getCurrentUserRewardPointWallet($user_id);
        
            if ($type == 'credit') {
                $balance += $amount;
				if(!$details)
				$details='Get Reward point';
            } else if ($type == 'debit') {
                $balance -= $amount;
				if(!$details)
				$details='Redeem Check';
            }
            if ($wpdb->insert("{$wpdb->base_prefix}woo_wallet_transactions", array('blog_id' => $GLOBALS['blog_id'], 'user_id' => $user_id, 'type' => $type, 'amount' => $amount, 'balance' => $balance, 'currency' => get_woocommerce_currency(), 'details' => $details), array('%d', '%d', '%s', '%f', '%f', '%s', '%s'))) {
 ...
 add_action( 'gform_after_submission_14', 'deductCheckRewardPoint', 10, 2 );

function deductCheckRewardPoint($entry)
{
	$user_id=$entry['created_by'];
	$amount=$entry[1];
	updateWalletBalanceReferral($user_id,$amount,'debit');
}


function getAllRedeem($user_id=null)
{
	 global $wpdb;
		if(!$user_id)
		$user_id=get_current_user_id(); 
	...
	$table=$wpdb->prefix."woo_wallet_transactions";
     $query="select * from $table where user_id=$user_id and type='debit' order by date DESC";
